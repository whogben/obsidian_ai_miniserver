...
- `obsidian( request:Request ) -> Response`
    ...
    - OpenAPI POST to `/api/obsidian` with typed request body `request:Request`
...
**openapi.json**
The OpenAPI spec generated by FastAPI, trimmed down for efficiency:
- Removed from properties
	- "title"s from all
	- "default"s from "const"
	- "type"s from "const" and "enum"
- Deduplicated inline oneOf+discriminator blocks (Pydantic inlines the same union in endpoint body and Batch/BatchResponse — these are extracted into named schemas and $ref'd)
- HTTPValidationError and ValidationError schemas
- "422" response entry
- The same property cleanup is applied to the MCP tool schema via `_clean_schemas()`
- Schema trimming reduces the MCP tool schema from ~3,622 tokens (raw Pydantic output) to ~1,776 tokens — a 51% reduction with no information loss
...
- whats good about it, including
	- "Maximum Control"
		...
		- Batch request enables multiple LLM actions in one-iteration
	...
	- "Maximum Token Efficiency"
		...
		- The entire tool schema uses ~ < X > tokens thanks to minimized docstrings, zero duplication or boilerplate
