...
def test_get_vault_info(vault, admin):
    ...
    assert resp[0].name == vault.vault_path.name
    assert resp[0].daily_notes_folder == "Dailies"
...
def test_move_file(vault, admin):
    resp = vault.obsidian(
        [MoveFile(old_path="notes/test.md", new_path="notes/renamed.md")],
        admin,
    )
    assert resp[0].kind == "success"
...
def test_list_files(vault, admin):
    ...
    names = [r.split(" | ")[0] for r in resp[0].results]
...
def test_upsert_user_delete(vault, admin):
    resp = vault.obsidian(
        [AdminUpsertUser(username="reader", delete_user=True)],
        admin,
    )
    assert resp[0].kind == "success"
...
def test_cannot_remove_admin(vault, admin):
    resp = vault.obsidian(
        [AdminUpsertUser(username="admin", delete_user=True)],
        admin,
    )
    assert resp[0].kind == "error"
...
def test_search_files(vault, admin):
    ...
    assert any("hello" in m for m in resp[0].results)
...
def test_search_files_with_path(vault, admin):
    ...
    assert any("other.md" in m for m in resp[0].results)
